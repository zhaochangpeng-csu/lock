...
**当前状态：** 服务、文件写入、processed 标记和生命周期已在本机验证；最新自动触发代码待开发板 SSH 恢复后同步复测。
...
1. 开发板 SSH 当前在密钥交换阶段主动关闭连接，最新改动尚未同步。
...
1. 恢复SSH并同步最新代码。
2. 验证陌生人3秒自动事件和融合失败事件。
3. 验证 Pipecat 通过 `aplay` 的真实扬声器输出。
4. 复测完整 `start/restart/stop/crash` 和端口清理。
